  ...
  var osmLayer = L.tileLayer('https://tile.thunderforest.com/neighbourhood/{z}/{x}/{y}.png?apikey=523123eab9074addb51cc220ddc9df2d', {
...
function zoomToFeature(e) {
	...
	fetch('/api/combine?commune='+ e.target.feature.properties.KOMNAVN +'&year=2018&data=information')
	.then(
	function(response){
		response.json().then(function(data) {
			console.log(data)
			...
			longitude.toString()
			let replacedLatitude = latitude.replace(",", ".")
...
info.update = function (props) {
    this._div.innerHTML = '<h4>Vælg kommune</h4>' +  (props ?
        '<b>' + props.KOMNAVN + '</b><br />' +' Kommune ID: ' + props.KOMKODE
        : 'Tryk på en kommune brah');
};
